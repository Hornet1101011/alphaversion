...
exports.addResponse = exports.updateInquiry = exports.getInquiryById = exports.getAllInquiries = exports.createInquiry = exports.getMyInquiries = void 0;
const getMyInquiries = async (req, res, next) => {
    try {
        ...
        const inquiries = await Inquiry_1.Inquiry.find({ username: user.username, barangayID: user.barangayID })
            .sort({ createdAt: -1 });
        res.json(inquiries);
    }
    catch (error) {
    ...
    try {
        ...
        if (resolvedUsername) {
            try {
                ...
                const possibleResident = await User_1.User.findOne({
                    role: 'resident',
        ...
        if (!resolvedUsername && (subject || message)) {
            try {
                ...
                const possible = await User_1.User.findOne({
                    role: 'resident',
        ...
        const inquiry = new Inquiry_1.Inquiry({
            subject,
            message,
            type: type || 'General',
        ...
        if (req.files && Array.isArray(req.files) && req.files.length > 0) {
            ...
            inquiry.attachments = req.files.map(f => ({
                filename: f.originalname,
                path: f.path,
                url: `${proto}://${host}/uploads/inquiries/${path_1.default.basename(f.path)}`,
...
const getInquiryById = async (req, res, next) => {
    try {
        const inquiry = await Inquiry_1.Inquiry.findById(req.params.id)
...
const updateInquiry = async (req, res, next) => {
    try {
        const inquiry = await Inquiry_1.Inquiry.findByIdAndUpdate(req.params.id, { ...req.body }, { new: true });
        if (!inquiry) {
            return res.status(404).json({ message: 'Inquiry not found' });
        }
        res.json(inquiry);
...
const addResponse = async (req, res, next) => {
    ...
    module.exports = {
        ...
        getMyInquiries: exports.getMyInquiries,
    };
    try {
        const inquiry = await Inquiry_1.Inquiry.findById(req.params.id);
        if (!inquiry) {
        ...
        if (req.files && Array.isArray(req.files) && req.files.length > 0) {
            const host = req.get('host');
            const proto = req.protocol;
            responseEntry.attachments = req.files.map(f => ({
                filename: f.originalname,
                path: f.path,
                url: `${proto}://${host}/uploads/inquiries/${path_1.default.basename(f.path)}`,
                contentType: f.mimetype,
        ...
        let resident;
        try {
            resident = await User_1.User.findOne({ username, barangayID, role: 'resident' });
        }
        catch (userErr) {
        ...
        if (resident) {
            index_1.io.to(resident._id.toString()).emit('inquiryResponse', {
