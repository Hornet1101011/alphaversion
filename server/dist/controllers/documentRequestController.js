...
exports.updatePaymentStatus = exports.processDocumentRequest = exports.getAllDocumentRequests = exports.getMyDocumentRequests = exports.createDocumentRequest = exports.generateFilledDocument = exports.previewFilledDocument = void 0;
// Preview filled document without saving request
const previewFilledDocument = async (req, res) => {
    try {
...
exports.previewFilledDocument = previewFilledDocument;
const DocumentRequest_1 = require("../models/DocumentRequest");
const mongoose_1 = __importDefault(require("mongoose"));
// @ts-ignore
const mongodb_1 = require("mongoose/node_modules/mongodb");
...
const generateFilledDocument = async (req, res) => {
    try {
        const { id } = req.params;
        const documentRequest = await DocumentRequest_1.DocumentRequest.findById(id);
        if (!documentRequest) {
        ...
        const bucket = new mongodb_1.GridFSBucket(db, { bucketName: 'templates' });
            ...
            try {
                ...
                try {
                    if (!documentRequest.transactionCode) {
                        documentRequest.transactionCode = await require('../utils/transactionCode').generateUniqueTransactionCode(DocumentRequest_1.DocumentRequest);
                        try {
                            await documentRequest.save();
                        }
                        catch (saveErr) {
                            console.error('Error saving transactionCode to documentRequest:', saveErr);
                        }
                    }
                ...
                try {
                    ...
                    if (tx) {
                        ...
                        if (docXmlFile) {
                            ...
                            try {
                                const qrBuffer = await QRCodeService_1.QRCodeService.generateDocumentQRBuffer(documentRequest._id?.toString() || tx, true);
                                // Add image to zip
  ...
  <w:drawing>
    <wp:inline>
      ...
      <a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">
          ...
          <pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture">
            ...
            <pic:spPr>
              ...
              <a:prstGeom prst="rect"><a:avLst/></a:prstGeom>
                ...
                try {
                    const filesDb = mongoose_1.default.connection.db;
                    if (!filesDb) {
                        console.error('No DB connection available for GridFS upload');
                    }
                    const documentsBucket = new mongodb_1.GridFSBucket(filesDb, { bucketName: 'documents' });
                    ...
                    readable.pipe(uploadStream)
                        ...
                        try {
                            ...
                            documentRequest.filledFileId = uploadStream.id;
                            await documentRequest.save();
                            // Emit socket event notifying about completed generation
                            try {
                                index_1.io.emit('documentGenerated', { requestId: id, filledFileId: uploadStream.id, filename });
                            }
                            catch (e) { /* ignore */ }
                        }
                        catch (err) {
        ...
        res.status(500).json({ message: 'Error generating filled document', error: (error?.message) || error });
    }
};
...
const createDocumentRequest = async (req, res) => {
    try {
        ...
        const documentRequest = new DocumentRequest_1.DocumentRequest(docReqData);
...
const getMyDocumentRequests = async (req, res) => {
    try {
        ...
        const user = req.user;
        const documentRequests = await DocumentRequest_1.DocumentRequest.find({ username: user.username, barangayID: user.barangayID })
...
exports.getMyDocumentRequests = getMyDocumentRequests;
const getAllDocumentRequests = async (req, res) => {
    try {
        ...
        if (type)
            query.type = type;
        const documentRequests = await DocumentRequest_1.DocumentRequest.find(query)
            .sort({ dateRequested: -1 })
...
const processDocumentRequest = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (status === 'approved') {
            ...
            index_1.io.emit('documentStatusUpdate', {
                requestId: id,
                status,
                remarks,
                processedBy,
...
exports.processDocumentRequest = processDocumentRequest;
...
async function getTemplateContentByFileId(fileId) {
